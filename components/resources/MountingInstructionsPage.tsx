...
import { ArrowLeft, Download, Scissors, Link as LinkIcon, Box, CircleDot } from 'lucide-react';
...
const MountingInstructionsPage: React.FC<Props> = ({ onBack }) => {
  ...
  const handleDownloadPNG = () => {
    ...
    const drawFooter = () => {
      ...
      ctx.fillText(`THYNKLAB 2026 © • YEAR 4 ENGINEERING • COUPLING GUIDE`, width / 2, height - 60);
      ctx.textAlign = 'left';
    };
    ...
    drawText("Coupling Instructions", margin, 320, "800 70px Montserrat, sans-serif", "#7C3AED");
    drawText("Powered Movement", margin, 400, "800 50px Montserrat, sans-serif", "#7C3AED");
    ...
    drawText("CONSTRUCTION STEPS", margin + 50, boxTop + 70, "800 28px Montserrat, sans-serif", "#64748B");
    
    const steps = [
      "1. Prepare the Cup: Flip cup and cut the bottom off.",
      "2. Align Coupler: Insert coupler into the wider rim.",
      "3. Bridge to Cart: Tape coupler to chassis front/back.",
      "4. Final Check: Ensure robot shell clears the cart chassis.",
      "5. Traction Check: The Sphero must touch the ground firmly."
    ];
    ...
    link.download = 'thynklab_coupling_instructions.png';
    ...
    <div className="flex flex-col h-full bg-slate-100 overflow-y-auto animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="flex-1 p-8 flex justify-center pb-20">
         <div className="bg-white shadow-2xl w-full max-w-[210mm] aspect-[1/1.414] p-[20mm] flex flex-col relative select-none">
            ...
            <div className="mb-10">
              <h2 className="text-3xl font-extrabold text-purple-600 leading-tight">Coupling Instructions<br/>Powered Movement</h2>
            ...
            <div className="bg-slate-50 border-2 border-slate-200 rounded-3xl p-8 mb-12 flex-1">
               ...
               <div className="space-y-8">
                 {[
                   { icon: Scissors, text: "Prepare the Cup: Flip cup upside down and cut the bottom off." },
                   { icon: Box, text: "Align Coupler: Insert coupler into the rim of the cup opening." },
                   { icon: LinkIcon, text: "Bridge to Cart: Securely tape the coupler to the chassis." },
                   { icon: CircleDot, text: "Placement: Dock the Sphero. It must touch the floor." }
                 ].map((step, i) => (
                   <div key={i} className="flex items-start gap-6">
                     <div className="w-10 h-10 rounded-xl bg-purple-100 text-purple-600 flex items-center justify-center shrink-0"><step.icon size={20} /></div>
                     <p className="text-slate-800 font-bold text-lg leading-snug">{step.text}</p>
                   </div>
                 ))}
               </div>
            </div>
            <div className="mt-auto border-t-2 border-slate-100 pt-6 flex justify-between items-center text-[10px] font-bold uppercase tracking-wider text-slate-400">
               ...
               <span>Year 4 • Powered Movement</span>
            </div>
         </div>
      </div>
    </div>
